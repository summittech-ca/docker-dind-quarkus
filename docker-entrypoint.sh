...
start_docker() {
  ...
  if [[ -n "${REGISTRY_MIRROR:-}" ]]; then
    ...
    if ! [[ $REGISTRY_MIRROR =~ "http" ]]; then
      echo "REGISTRY_MIRROR ${REGISTRY_MIRROR} does not start with (http|https), prefixing https://"
      REGISTRY_MIRROR="https://${REGISTRY_MIRROR}"
    fi
    docker_opts+=" --registry-mirror=${REGISTRY_MIRROR}"
  elif [[ -z "${DOCKER_IO_USERNAME:-}" ]] || [[ -z "${DOCKER_IO_PASSWORD:-}" ]]; then
